...
| ✅ 다크모드 대비 개선 | 전체 배경 CSS 변수화, 헤딩·텍스트 오버라이드, 캐시버스팅 | 완료 |
| ✅ 노트 내 키워드 검색 | 하이라이트 + 매치 수 + ↑↓ 이동, Tab2·Tab3 모두 지원 | 완료 |
| ✅ Gen-modal 실시간 미리보기 | 생성 중 모달 내 텍스트 실시간 출력 + 자동스크롤 | 완료 |
| ✅ SVG 아이콘 안정화 | fetch → HTML 인라인 전환, 콜드스타트 시 아이콘 소실 해결 | 완료 |
