...
| ✅ 다크모드 대비 개선 | 전체 배경 CSS 변수화(`var(--surface)`), 헤딩·텍스트 오버라이드, 캐시버스팅 | 완료 |
| ✅ 노트 내 키워드 검색 | TreeWalker 하이라이트, 매치 수·↑↓ 탐색, Tab2·Tab3 공통 | 완료 |
| ✅ Gen-modal 실시간 미리보기 | 모달 내 `#gm-preview` — LLM 생성 텍스트 실시간 출력 + 자동스크롤 | 완료 |
| ✅ SVG 아이콘 안정화 | fetch 방식 → HTML 인라인으로 전환, 콜드스타트 아이콘 소실 해결 | 완료 |
