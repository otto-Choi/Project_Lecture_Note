...
> 출처: `docs/렉쳐노트 피드백.txt` — 외부 사용자 5인 피드백 중 아직 반영되지 않은 항목

| 피드백 | 제안자 | 개선 방향 |
|---|---|---|
| "강의 등록과 노트 생성의 차이가 바로 와닿지 않았다. 강의 등록은 학기 초 1회, 노트 생성은 매주 반복이라는 안내가 더 분명하면 좋겠다." | 이종택 (중앙대 경영 20) | 강의 등록 카드에 "학기 초 1회" 뱃지·설명 문구 강화, 노트 생성 탭에 "매주 반복" 안내 추가 |
| "생성된 노트가 길어질 경우 모바일 화면에서 빽빽하게 보인다. 핵심 요약, 교수님 강조, 예상 문제 부분이 더 확실히 나뉘어 보이면 좋겠다." | 소은규 (중앙대 경영 20) | 노트 뷰어 CSS에서 섹션 헤딩 간격·구분선 강화, Overview/Integrated Notes/Self-Exam 섹션별 배경색 차별화 |
