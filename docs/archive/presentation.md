...
### 경영전략 9팀 | 2026-06-07 업데이트 | **베타 빌드 완료 + Android 패키징 완료**
...
| 11단계 | 2026-05-15 | **베타 빌드** — Pipeline A UI, Profile B, 아이콘 스프라이트 |
| 12단계 | 2026-05-15 | **베타 기능 완성** — Gen-banner, 아이디/비번 찾기, 구독 플랜·언어·가입 정보 화면, 이용약관, 비밀번호 강도 게이지 |
| **13단계** | **2026-06-02** | **다크모드 대비 개선** — 전체 배경 CSS 변수화, 사용자 피드백 5인 반영 |
| **14단계** | **2026-06-02** | **Gen-modal + 노트 섹션 구분 강화** — 파이프라인 팝업 UX, 사용 안내 배너, h1 시각 구분 |
| **15단계** | **2026-06-02** | **SVG 인라인 전환 + Gen-modal 실시간 미리보기** — 콜드스타트 아이콘 소실 해결, gm-preview 실시간 출력 |
| **16단계** | **2026-06-07** | **배포 Railway 재전환** — `https://lecturenote.up.railway.app` |
| **17단계** | **2026-06-07** | **Android 패키징** — Capacitor APK + PWABuilder TWA (APK/AAB + Google Play 서명키) |
...
### 4-2. 파이프라인 스텝 인디케이터 (Pipeline A)
- 5단계 진행 상태 시각화 (연결선·서브텍스트·경과 시간 카운터)
- STT 완료 → SSE 시작 시 스텝 자동 전환
- 현재 처리 중인 단계 펄스 애니메이션

### 4-3. Gen-modal 팝업 + 실시간 미리보기 *(13~15단계 신규)*
- 노트 생성 시작 → 파이프라인 팝업 자동 표시
- "백그라운드에서 진행" 클릭 → 상단 배너로 최소화
- 배너 클릭 → 팝업 재표시
- `#gm-preview` 영역: LLM이 생성하는 텍스트를 모달 내에서 실시간 출력 + 자동 스크롤

### 4-4. 노트 내 키워드 검색 *(13단계 신규)*
- TreeWalker로 DOM 전체 텍스트 노드 순회 → `<mark>` 하이라이트
- 현재 포커스(주황) / 나머지 매치(노랑) 구분
- 매치 수 표시, ↑↓ 버튼 및 Enter/Shift+Enter 탐색
- Tab2(생성 결과) + Tab3(노트 조회) 공통 지원

### 4-5. 다크모드 대비 개선 *(13~15단계 신규)*
- 전체 배경색 CSS 변수화 (`var(--surface)`, `var(--bg)`)
- 외부 사용자 피드백 5인 반영: 입력창·카드·헤딩·버튼 대비 강화
- 다크/라이트 전환이 모든 컴포넌트에 자동 적용

### 4-6. 프로필 화면 (Profile B)
- 그라디언트 헤더 + 아바타 이니셜 자동 생성
- 강의 수·노트 수·이번 주 생성 노트 수 통계
- 프로필 편집 (이름·학교·전공), 비밀번호 변경, 계정 삭제
- 가입 정보 / 구독 플랜 (Free/Pro/Team) / 언어 설정 화면

### 4-7. 회원가입 강화
- 이메일 필드 (비밀번호 찾기 연락처)
- 비밀번호 강도 게이지: 8자/영문/숫자/특수문자 기준 0~4단계 시각화
- 이용약관 동의: 필수 미동의 시 가입 버튼 비활성화

### 4-8. STT 성능 최적화
- `thinking_budget=0` 적용으로 STT·메타 분석 응답 속도 향상
- `asyncio.gather`로 PDF 파싱·STT 병렬 처리 → 전체 대기 시간 단축

### 4-9. 원클릭 강의 등록 + 실시간 스트리밍 노트 생성
- 강의계획서 PDF → Gemini 커리큘럼 분석 (Step 0), 과목명·교수명 자동 추출
- 음성(인앱 녹음 or 파일) + PDF + 필기 동시 업로드 → SSE 스트리밍 노트 생성
- 스트리밍 중 자동 스크롤 (수동 스크롤 시 자동 멈춤)

### 4-10. Android 앱 *(17단계 신규)*
- **Capacitor APK**: 시연·사이드로드용 디버그 빌드 (`com.kbslab.lecturenote`)
- **PWABuilder TWA**: Play Store 등록용 릴리즈 APK + AAB (`com.cau.lecturenote`)
- 앱 내 ⚙ 설정 시트로 API URL 즉시 변경 가능 (시연 환경 전환)
...
| **백엔드** | FastAPI + Python | REST API (19개), SSE 스트리밍 |
...
| **모바일** | Capacitor 6 + PWABuilder TWA | Android APK/AAB 빌드 완료 |
...
```
[브라우저 / Android WebView - index.html]
...
| GET    | `/api/auth/stats` | 강의·노트 통계 (이번 주 포함) |
...
**Step 1 — 앱 접속**
- 웹: `https://lecturenote.up.railway.app`
- 안드로이드: `LectureNote.apk` 사이드로드 설치

**Step 2 — 회원가입 / 로그인**
1. 온보딩 화면 → 회원가입 (2-step)
2. 로그인 → 메인 앱 진입
...
4. **Gen-modal 팝업에서 Pipeline A 5단계 진행 + 실시간 미리보기 확인**
5. "백그라운드에서 진행" → 상단 배너로 최소화 후 다른 탭 이동 가능

**Step 5 — 노트 조회 / 검색**
1. 노트 탭 → 강의 선택 → 주차 노트 목록
2. 노트 클릭 → 편집·다운로드
3. **검색 버튼 → 키워드 입력 → 하이라이트 + ↑↓ 탐색**

**Step 6 — 프로필**
1. 우상단 프로필 아이콘 → Profile B 화면
2. 통계·플랜·언어 설정 확인
...
| 처리 현황 | 무응답 대기 | **Pipeline A + Gen-modal** |
| 데이터 격리 | 공유 저장소 | **사용자 인증 + 개인화** |
| 노트 내 탐색 | 스크롤만 가능 | **키워드 검색 + 하이라이트** |
...
| ✅ 완료 | 백그라운드 노트 생성 + Gen-modal | 완료 |
| ✅ 완료 | 아이디/비밀번호 찾기 | 완료 |
| ✅ 완료 | 구독 플랜·언어·가입 정보 화면 | 완료 |
| ✅ 완료 | 회원가입 강화 (이메일·비밀번호 강도·이용약관) | 완료 |
| ✅ 완료 | 다크모드 대비 개선 (피드백 5인 반영) | 완료 |
| ✅ 완료 | 노트 내 키워드 검색 | 완료 |
| ✅ 완료 | Gen-modal 실시간 미리보기 (gm-preview) | 완료 |
| ✅ 완료 | SVG 아이콘 인라인 전환 (콜드스타트 소실 해결) | 완료 |
| ✅ 완료 | Railway 배포 | 완료 |
| ✅ 완료 | Android APK + AAB (Capacitor + PWABuilder) | 완료 |
| 높음 | **다중 파일 업로드** (PDF·음성 여러 개 동시) | 대기 |
| 높음 | **노트 추가 요청** (생성된 노트 기반 후속 질의) | 대기 |
| 중간 | 이미지 입력 (칠판 필기 사진 → Gemini Vision OCR) | 대기 |
| 중간 | 처리 메타데이터 표시 (소스 구성·처리 시간) | 대기 |
| 중간 | Play Store 등록 (AAB 빌드 완료, 등록 절차 진행 필요) | 대기 |

---

## 11. 확장 가능성 — 강의 노트를 넘어서

이 파이프라인의 본질은 강의 처리가 아니라 **비정형 멀티모달 입력 → 구조화 출력** 아키텍처다.  
Step 0 컨텍스트 문서만 교체하면 동일한 코드가 다른 도메인에 즉시 적용된다.

| 도메인 | Step 0 교체 | 출력 |
|--------|------------|------|
| **회의 요약** | 안건 목록 | 결정 사항·액션 아이템·책임자 |
| **문서 요약** | 검토 체크리스트 | 핵심 조항·리스크 요약 |
| **사내 온보딩** | 온보딩 가이드 | 부서별 핵심 내용·FAQ |
| 감사·실사 | 감사 체크리스트 | 리스크 항목 보고서 |
...
- Android APK: `package/LectureNote - Google Play package/LectureNote.apk`
