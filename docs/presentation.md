...
| **12단계** | **2026-05-15** | **베타 기능 완성** — Gen-banner, 아이디/비번 찾기, 구독 플랜·언어·가입 정보 화면, 이용약관, 비밀번호 강도 게이지, Render 배포 |
...
- 가입 정보 화면: 계정 상세 정보 조회
- 구독 플랜 화면: Free / Pro / Team 비교 카드
- 언어 설정 화면: 한국어/English/日本語/中文 선택

### 4-4. 백그라운드 노트 생성 *(신규, Gen-banner)*
- 노트 생성 중 상단 배너로 진행 상태 표시 — 다른 탭을 자유롭게 이동 가능
- 파이프라인 단계 실시간 업데이트 (음성 처리 → 자료 통합 → 노트 생성)
- 경과 시간 카운터, 완료 시 "보기 →" 버튼으로 해당 노트 즉시 이동

### 4-5. 회원가입 강화 *(신규)*
- 이메일 필드 (비밀번호 찾기 연락처)
- 비밀번호 강도 게이지: 8자/영문/숫자/특수문자 기준 0~4단계 시각화
- 이용약관 동의: [필수] 이용약관·개인정보처리방침, [선택] 마케팅 — 필수 미동의 시 가입 버튼 비활성화

### 4-6. 아이디/비밀번호 찾기 *(신규)*
- 표시 이름 + 학교명으로 아이디 조회
- 등록 이메일로 임시 비밀번호 안내 (데모 단계, 실제 발송 미구현)
