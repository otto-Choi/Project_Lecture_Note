...
> **피드백 출처**: `docs/렉쳐노트 피드백.txt` (외부 사용자 5인) 및 개발 중 추가 요청

### 변경 1: 다크모드 색상 대비 수정

**피드백**:
- *"다크모드 환경에서는 일부 화면의 시인성이 부족하게 느껴질 수 있어 색 대비 조정이 필요하다."* — 이병헌 (중앙대 경영 20)
- *"다크모드에서 일부 글자나 버튼 대비가 약하게 느껴질 수 있다는 점이다. 실제 사용자가 오래 보면 색 대비가 조금 더 강한 편이 좋을 것 같다."* — 오수정 (서울여대 국제학과 20)
...
**피드백**: *"백그라운드에서 실행하는 방식 자체는 좋으나, 과정이 시작됐는지 인지하지 못함. 큰 팝업을 일단 띄우고 [백그라운드에서 진행] 버튼을 누르면 상단바에 나타나며, 상단바를 다시 누르면 그 창이 뜨도록."*
...
**피드백**: *"생성된 노트에서의 검색 기능도 넣을 수 있으려나?"*
...
## 14단계: Gen-modal 버그 수정 · 사용성 안내 · 노트 섹션 구분 (2026-06-02)

### 변경 1: Gen-modal 초기 흐름 버그 수정

**문제**: `showGenBanner()` 에서 배너(`#gen-banner`)를 즉시 `display:flex`로 표시하여, 생성 시작 시 모달과 배너가 동시에 노출되는 버그. 의도한 흐름(시작 시 모달만 → "백그라운드에서 진행" 클릭 시 배너 등장)이 동작하지 않았음.

**수정** (`public/assets/app.js`):
- `showGenBanner()`: `b.style.display = 'flex'` → `b.style.display = 'none'` (배너 초기 숨김)
- `hideGenModal()`: `$('gen-banner').style.display = 'flex'` 추가 (모달 최소화 시 배너 등장)
- `completeGenBanner()`: 배너가 표시된 상태에서 완료될 경우 배너에도 done 처리 추가

**최종 흐름**:
1. 생성 시작 → 모달만 표시 (배너 숨김)
2. "백그라운드에서 진행" 클릭 → 모달 닫힘, 배너 등장
3. 배너 클릭 → 모달 재표시
4. 완료 → 모달 내 "노트 보기 →" 활성화, 배너가 보이면 배너도 녹색 완료 처리

---

### 변경 2: Tab2 사용 안내 추가

**피드백**: *"강의 등록과 노트 생성의 차이가 바로 와닿지 않았다. 강의 등록은 학기 초 1회만 하는 과정이고, 노트 생성은 매주 반복하는 과정이라는 안내가 조금 더 분명하면 좋겠다."* — 이종택 (중앙대 경영 20)

**수정** (`public/index.html`):
- Tab2 "강의 선택" 카드 타이틀에 `"매주 반복"` 서픽스 뱃지 추가
- 카드 하단에 `.info-banner` 추가: "강의 등록(Tab 1)은 학기 초 1회만 합니다. 이 탭은 매주 수업 후 사용하세요."

---

### 변경 3: 노트 h1 섹션 시각 구분 강화

**피드백**: *"생성된 노트가 길어질 경우 모바일 화면에서 다소 빽빽하게 보일 수 있다. 핵심 요약, 교수님 강조, 예상 문제 같은 부분이 더 확실히 나뉘어 보이면 읽기 편할 것 같다."* — 소은규 (중앙대 경영 20)

**수정** (`public/assets/styles.css`):
- `.note-view h1`: 기존 `border-bottom` 방식 → `background: var(--blue-soft)` + `border-left: 3px solid var(--blue)` + `padding: 8px 10px` + `border-radius: 6px`
- `.note-view h1:first-child`: 첫 섹션은 상단 여백 최소화
- 다크모드 h1 오버라이드: 배경·좌측 바 색상 별도 지정
